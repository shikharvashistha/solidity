...
void Parser::fetchDebugDataFromComment()
{
	...
	match_results<string_view::const_iterator> tagMatch;

	langutil::SourceLocation sourceLocation = m_debugDataOverride->location;

	while (regex_search(commentLiteral.cbegin(), commentLiteral.cend(), tagMatch, tagRegex))
	{
		solAssert(tagMatch.size() == 2, "");
		commentLiteral = commentLiteral.substr(static_cast<size_t>(tagMatch.position() + tagMatch.length()));

		if (tagMatch[1] == "@src")
		{
			if (auto parseResult = parseSrcComment(commentLiteral, commentLocation))
